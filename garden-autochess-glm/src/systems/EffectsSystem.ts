...
export class EffectsSystem {
  ...
  private readonly beamGeo = new THREE.CylinderGeometry(0.06, 0.06, 1, 6);
  ...
  private readonly beamMat = new THREE.MeshBasicMaterial({ transparent: true, blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  // A short-lived vertical billboard beam (used for lightning / column hits).
  spawnBeam(position: THREE.Vector3, color: string, height: number, duration: number): void {
    ...
    mesh.scale.y = height;
    this.group.add(mesh);
    this.beams.push({ mesh, life: duration, maxLife: duration });
  }
  ...
  update(delta: number): void {
    ...
    for (let i = this.beams.length - 1; i >= 0; i -= 1) {
      ...
      (beam.mesh.material as THREE.MeshBasicMaterial).opacity = (1 - t) * 0.9;
      beam.mesh.scale.x = beam.mesh.scale.z = 1 - t * 0.4;
      if (beam.life <= 0) {
